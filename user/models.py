...
def create_events_from_excel(file_path):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):  
        ...
        try:
            ...
            event_obj =event.objects.create(author=User.objects.get_or_create(username='admin')[0],category=CATEGORY, event_date=new_date_str, event_title=TITLE,event_bullet_1=BULLET1, event_bullet_2=BULLET2, event_bullet_3=BULLET3,event_bullet_4=BULLET4, event_bullet_5=BULLET5, source1_url=source1_url, source2_url=source2_url, source3_url=source3_url)
            event_obj.tags.set(tags_list)

            # if source1_url:
            #     screenshot = asyncio.run(capture_screenshot(source1_url))
            #     image_name = f"{event_obj.id}-source1.png"
            #     image_path = os.path.join(settings.MEDIA_ROOT, image_name)
            #     print(screenshot,"================================================")
            #     print(image_path,"************************************")
            #     with open(image_path, 'wb') as f:
            #         f.write(screenshot)
            #     event_obj.source1_image = image_name
            #     event_obj.save()
        except ValueError:
            pass
...
# async def run_capture_screenshot(source1_url):
#     screenshot = await capture_screenshot(source1_url)
...
#     source1_url = 'https://www.example.com'
#     loop.create_task(run_capture_screenshot(source1_url))
